...
| **Discovery** | **IP Scanner** (nmap-style host sweep, port scan, service ID) · **DHCP/BOOTP** (DISCOVER + option decode, rogue-server detection, **BOOTP address assignment** — DISCOVER→REQUEST→ACK for a MAC) · **PROFINET DCP** (PRONETA-style Identify-All discovery, a **segment topology map**, and **DCP Set** to commission station name / IP / subnet / gateway — ARM-gated) |
...
```bash
npm test     # 73 tests: contract, rules, evidence, the double-gate, and every
...
- **§7 Discovery tier** → `drivers/ipscan.js` (nmap-style host sweep + port scan + service ID, concurrency-pooled and bounded), `drivers/dhcp.js` (DHCP DISCOVER + BOOTP/option decode + rogue-server detection, plus a BOOTP-style address assignment — the DORA DISCOVER→REQUEST→ACK cycle — as an ARM-gated write), `drivers/profinet-dcp.js` (PRONETA-style DCP Identify-All discovery with duplicate-name / unconfigured-IP commissioning verdicts, a `browse` verb that renders the segment as a topology map, and a DCP **Set** write that reconfigures station name / IP / subnet / gateway behind the double-gate with an Identify read-back; the DCP codec runs over a UDP test harness — real DCP is raw Ethernet, declared `requires_l2`).
